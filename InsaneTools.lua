...
if enable_autoupdate then
    ...
    if updater_loaded then
        ...
        if autoupdate_loaded then
            Update.json_url = "https://raw.githubusercontent.com/ImInsane1337/insanetools/main/version.json" .. tostring(os.clock())
            Update.prefix = "[" .. string.upper(thisScript().name) .. "]: "
            Update.url = "https://github.com/ImInsane1337/insanetools"
        end
    end
end
...
colors = imgui.CreateTextureFromFile(getGameDirectory() .. "\\moonloader\\InsaneTools\\color.png")
authors = imgui.CreateTextureFromFile(getGameDirectory() .. "\\moonloader\\InsaneTools\\authors.png")
...
local cfg = inicfg.load({
    main = {
		...
		nead = 60,
		mdm = 5,
		sendgoodplay = tostring(u8'/aad Администрация YouTube RP желает Вам приятной игры на просторах нашего сервера!'),
		cmdHouse = 'ohome',
		cmdBusiness = 'obiz',
		...
		infopunish1 = tostring(u8'Ссылочка на наш форум - forum.youtube-rp.space | Приятной игры!'),
		kapchas = '',
		present = '',
		formis = '',
		akkpass = '',
...
local selector_list = {
fa.ICON_FA_USER_COG.. u8' Главная',
fa.ICON_FA_PALETTE.. u8' Изменить цвет',
fa.ICON_FA_KEYBOARD.. u8' Назначение клавиш',
fa.ICON_FA_BAN.. u8' Выдача наказаний',
fa.ICON_FA_ROBOT.. u8' Панель рекона [Beta]',
fa.ICON_FA_DESKTOP.. u8' Чекеры',
fa.ICON_FA_SKULL_CROSSBONES.. u8' КиллЛист',
fa.ICON_FA_CLIPBOARD.. u8' Быстрые команды',
fa.ICON_FA_HANDSHAKE.. u8' Формы в админ-чате',
fa.ICON_FA_SKULL.. u8' Трейсера пуль',
...
buffer99 = imgui.ImBuffer(tostring(cfg.main.giveaways), 256)
...
buffer29 = imgui.ImBuffer(tostring(cfg.main.gwp), 256)
buffer29.v = string.gsub(tostring(buffer29.v), '"', '')
...
local when_giveaway = imgui.ImInt(1)
local formsall = 0
...
changelog = {
    [0] = {
        ...
        ['info'] = {
            ...
            'Добавлено удаление флуда от /arep',
            ...
            'Список команд перенесён в отдельное окно (/ath)',
	...
	[2] = {
        ...
        ['info'] = {
			...
			'Добавлена автоматическая ловля репорта с тремя способами ловли: по тексту в чате/по надписи "REPORT ++"(Скоро)/обычный флуд /arep',
			...
			'Добавлена рандомная генерация капчи в /captcha',
...
'Теперь конфиг скрипта хранится в папке скрипта, а не в папке config для всего мунлоадера (путь: moonloader/config/InsaneTools/AdminTools.ini) '
        },
        ['patches'] = {
		['show'] = false,
            ['info'] = {
...
GiveawayWindow = imgui.ImBool(false)
...
local function createNewJsonTable(table)
	if not doesFileExist(getWorkingDirectory().."\\InsaneTools\\"..table[2]..".json") then
	   	local f = io.open(getWorkingDirectory().."\\InsaneTools\\"..table[2]..".json", 'w+')
	   	if f then
		  	f:write(encodeJson(table[1])):close()
		end
   	else
	   	local f = io.open(getWorkingDirectory().."\\InsaneTools\\"..table[2]..".json", "r")
	   	if f then
...
pathOffLog = getWorkingDirectory().."\\InsaneTools\\Логи\\AutoOpraLogs.txt"
log_full = {}
          ...
          if tonumber(param) ~= tonumber(cfg.Settings.mode) then
            for k, v in pairs(messageChangeMode) do
              if (param + 1) == k then
                ...
                cfg.Settings.mode = (k - 1); inicfg.save(cfg, "InsaneTools\\AdminTools.ini"); break
              end
            end
          else
...
local function get_id_by_nick(nick)
	...
	if cfg.main.gm then
		...
		if cfg.main.airbrake then 
			...
			printStringNow("~w~AirBreak "..(enAirBrake and "~b~OFF" or "~g~ON"), 2000)
		...
		if isKeysDown(facceptform.v) and not sampIsChatInputActive() and not sampIsDialogActive() and not isPauseMenuActive() and not isSampfuncsConsoleActive() and cfg.main.trueform then
			...
			lua_thread.create(function()
				...
				local f = io.open(getGameDirectory().."//moonloader//InsaneTools//AcceptForms.txt", 'a')
				...
				autoformaq = "false"
                                wait(999)
				sampSendChat("/a [FormaIT] Форма принята")
				acceptforma = "true"
			end)
			end
		end
...
local X, Y = getScreenResolution()
	...
	if MainWindow.v then
        ...
        imgui.Begin(fa.ICON_FA_TOOLS..u8' InsaneTools | YouTube RP '..fa.ICON_FA_TOOLS, MainWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoSavedSettings) 	
	   ...
	   if  imgui.Button(fa.ICON_FA_SAVE..u8'', imgui.ImVec2(54, 27)) then
			...
			inicfg.save(cfg, "InsaneTools\\AdminTools.ini")
       end
	   imgui.Hint('saves', u8'Сохранить настройки')
		...
		if menusel.v == 1 then
			...
			if cfg.main.mylvladmin == 12 then 
            ...
            if cfg.main.mylvladmin == 13 then 
				imgui.Text(u8'Ваш LVL фулл доступа:')
				imgui.SameLine()
				imgui.PushItemWidth(20)
				if imgui.InputInt('##asdasdadsadsdasdasda',ttttttt, 0) then 
           ...
           if imgui.Button(u8'Версия скрипта', imgui.ImBool(cfg.main.nobike)) then 
			sampAddChatMessage("", -1) 
		    sampAddChatMessage(tag ..  'Версия скрипта:{E78284} 1.2 Release', -1) 
		    sampAddChatMessage(tag ..  'Обновления у нас в чате!', -1) 
		    sampAddChatMessage(tag ..  'ВК:{E78284} bit.ly/InsaneTools', -1) 
		    sampAddChatMessage("", -1) 
			end 
			imgui.SameLine()
			...
			imgui.PopFont()
			imgui.Hint('ath11', u8'Вы никогда не будете падать с двухколесного транспорта')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath12', u8'Вы будете защищены от пуль, а транспорт от повреждений')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath13', u8'Аналог метлы, только в разы быстрее\nВкл./выкл.: R.SHIFT\nУскорение: Num +\nЗамедление:Num -')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath15', u8'Выполнение наказаний от мл. администрации')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath1', u8'Для работы данной функции укажите свой пароль от аккаунта в соответствующем пункте.\nПри подключении на сервер скрипт сам моментально введет пароль')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath16', u8'При авторизации на сервер скрипт автоматически введет пароль от админки')			
			...
			if imgui.RadioButton(u8'Скоро...', style_lovlyareporta, 2) then
				cfg.main.stylelovlyareporta = 2
			end
			if imgui.RadioButton(u8'Обычный флуд /arep', style_lovlyareporta, 3) then
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath2313842381', u8'При стрельбе у вас не будут тратиться патроны, они станут бесконечными')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath113', u8'ИПы в сообщениях будут заменяться таким образом: 93.хх.хх.186\nПолезная функция, ведь на опрах больше не будет ИПов игроков/админов')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath211221', u8'Отключает работу клавиши F5')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath18  ', u8'Весь текст будет отправляться в nRP (/b) чат')
            imgui.NextColumn()
			if imgui.ToggleButton(u8'Информация после ответа /arep', imgui.ImBool(cfg.main.inforeport)) then 
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath19', u8'Данная функция включает сообщение с информацией о ответе на последний репорт.')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath21', u8'Отключает работу клавиши F4, при частом попадании в больку отключите функцию и перезайдите')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath22', u8'Данная функция включает отображение худа/радара в реконе')
			...
			if imgui.ToggleButton(u8'Удалять флуд в чат от /arep', imgui.ImBool(cfg.main.deleteot)) then 
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath23', u8'Данная функция отключает сообщения в чате об отсутствии репорта')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath25', u8'Данная функция автоматически закрывает диалог о кике от античита (7+ LVL)')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath26', u8'Данная функция логгирует все принятые формы в отдельный файл, дабы сократить случаи подставы')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath2311384122381', u8'При стрельбе у вас автоматически будет скроллиться оружие, тем самым отсутствовать перезарядка')
			...
			imgui.TextDisabled(fa.ICON_FA_QUESTION_CIRCLE..u8'')
			imgui.PopFont()
			imgui.Hint('ath1122311384122381', u8'При прописывании /getip скрипт автоматически будет определять местоположение и выводить в /a либо локальный чат')
...
imgui.Text(u8'Задержка между отправкой строк:')
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//zawitabinder.txt", "w")
		...
		if imgui.BeginPopupModal(fa.ICON_FA_COGS..u8' Настройки | Множитель доната', MainWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize) then
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//donatebinder.txt", "w")
...
imgui.InputTextMultiline('##kotofqee1v16noyob', rulesbinder, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//rulesbinder.txt", "w")
...
imgui.InputTextMultiline('##eweqewqewdas', discordbinder, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//discordbinder.txt", "w")
...
imgui.InputTextMultiline('##eweqquuewqewdas', groupbinder, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//groupbinder.txt", "w")
		...
		if imgui.Button(u8'Собственный №1', imgui.ImVec2(200, 22)) then 
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//binder1.txt", "w")
...
imgui.InputTextMultiline('##kotofeevnoob', mybinder2, imgui.ImVec2(650, 250))
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//binder2.txt", "w")
...
imgui.Text(u8'Задержка между отправкой строк:')
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//binder3.txt", "w")
...
imgui.Text(u8'Задержка между отправкой строк:')
			...
			if imgui.Button(u8'Сохранить и закрыть', imgui.ImVec2(-1, 25)) then
				local file = io.open(getGameDirectory().."//moonloader//InsaneTools//binder4.txt", "w")
...
imgui.Separator()
	...
	if imgui.Button(u8("Добавить кнопку ответа в репорт"), imgui.ImVec2(-1, 25)) then
		...
		if menusel.v == 11 then
			...
			imgui.PushFont(fs20)
			imgui.CenterTextColoredRGB('Владелец скрипта:')
			imgui.PopFont()
			imgui.PushFont(fs80)
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('ImInsane').x) / 2)
			imgui.TextColored(imgui.ImVec4(rainbowtext(2)), 'ImInsane')
			imgui.PopFont()
			imgui.PushFont(fs16)
			imgui.CenterTextColoredRGB('Реализовано для проекта:')
			imgui.SetCursorPosX((imgui.GetWindowWidth() - imgui.CalcTextSize('YouTube RP | vk.com/youtuberp_news').x) / 2)
			imgui.TextColored(imgui.ImVec4(rainbowtext(2)), 'YouTube RP | vk.com/youtuberp_news')
			imgui.PopFont()
			imgui.PushFont(fs16)
						...
						sampSetCursorMode(CMODE_LOCKCAM)
                            ...
                            if isKeyDown(VK_LBUTTON) then 
                                ...
                                for k,v in pairs(J_) do 
									if doesFileExist(getWorkingDirectory().."\\InsaneTools\\"..J_[k][2]..".json") then 
										local f = io.open(getWorkingDirectory().."\\InsaneTools\\"..J_[k][2]..".json", "w+") 
										if f then 
...
imgui.RadioButton(u8"В общий чат (/aad)", when_captcha, 1)
imgui.Text(u8'Капча:')
imgui.SameLine()
imgui.PushItemWidth(-1) 
...
imgui.BeginChild("qeuqewiqweiqwe5", imgui.ImVec2(-1, 60), true)
		...
		imgui.PopFont()
        imgui.CenterText(u8('/aad [Captcha] Внимание! Капча: "'..u8:decode(cfg.main.kapchas))..u8('". Приз: '..u8:decode(cfg.main.present))..'!')
		if when_captcha.v == 1 then
		end
imgui.EndChild()
if imgui.Button(u8'Отправить капчу', imgui.ImVec2(-1, 25)) then
if when_captcha.v == 1 then
sampSendChat('/aad [Captcha] Внимание! Капча: "'..u8:decode(cfg.main.kapchas)..'". Приз: '..u8:decode(cfg.main.present)..'!')
sampAddChatMessage('{E78284}[AdminTools] {84A6E7}Капча была успешно отправлена!', -1)
end
end
imgui.End()
	end
	
            	if GiveawayWindow.v then
	        local sw, sh = getScreenResolution()
        imgui.SetNextWindowPos(imgui.ImVec2(sw / 2, sh / 2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.SetNextWindowSize(imgui.ImVec2(400, 290), imgui.Cond.FirstUseEver)

        imgui.Begin(fa.ICON_FA_COGS..u8' Настройки | Раздача', GiveawayWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoSavedSettings)
imgui.CenterText(u8'Куда отправлять Раздачу?')
imgui.RadioButton(u8"В общий чат (/aad)", when_giveaway, 1)
imgui.Text(u8'Слово для выигрыша:')
imgui.SameLine()
imgui.PushItemWidth(-1) 
imgui.InputText(u8"##40", buffer99) 
cfg.main.giveaways = string.format('%s', tostring(buffer99.v)) 
imgui.PopItemWidth()
imgui.Text(u8'Приз:')
imgui.SameLine()
imgui.PushItemWidth(-1) 
imgui.InputText(u8"##41", buffer29) 
cfg.main.gwp = string.format('%s', tostring(buffer29.v)) 
imgui.BeginChild("qeuqewiqweiqwe5", imgui.ImVec2(-1, 60), true)
		imgui.PushFont(fontsize)
		imgui.CenterText(u8'Предпросмотр:')
		imgui.PopFont()
		if when_giveaway.v == 1 then
		end
imgui.EndChild()
imgui.CenterText(u8('[РАЗДАЧА] Кто первый напишет в репорт: '..u8:decode(cfg.main.giveaways))..u8(' тот получит '..u8:decode(cfg.main.gwp))..'!')
if imgui.Button(u8'Отправить', imgui.ImVec2(-1, 25)) then
if when_giveaway.v == 1 then
sampSendChat('/aad [РАЗДАЧА] Кто первый напишет в репорт: '..u8:decode(cfg.main.giveaways)..' тот получит '..u8:decode(cfg.main.gwp)..'!')
sampAddChatMessage('{E78284}[AdminTools] {84A6E7}Раздача была успешно отправлена!', -1)
end
end
imgui.End()
	end
...
if ReportWindow.v then 
		...
		imgui.Begin(u8'Жалоба/Вопрос', ReportWindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoSavedSettings + imgui.WindowFlags.AlwaysAutoResize)
			...
			if not doesFileExist(getGameDirectory()..'//moonloader//InsaneTools//gps.json') then
			...
			imgui.OpenPopup(u8'Помощь по GPS')
				...
				local gps = io.open('moonloader\\InsaneTools\\gps.json', 'a+')
	...
	gps:close()
        ...
        if imgui.Button(v, imgui.ImVec2(510, 25)) then
			...
			if not doesFileExist(getGameDirectory()..'//moonloader//InsaneTools//commands.json') then
			...
			imgui.OpenPopup(u8'Список команд')
				...
				local commands = io.open('moonloader\\InsaneTools\\commands.json', 'a+')
...
lua_thread.create(function()
sampSendChat('/aad [Спавн Транспорта] Уважаемые игроки, через '..arg..' секунд произойдет респавн всего транспорта.')
wait(1000)
sampSendChat('/aad [Спавн Транспорта] Займите свой транспорт, в противном случае он пропадет!')
for i = arg,1,-1 do
wait(1000)
printStyledString("Spawn cars through: ~r~"..i, 1000, 5)
		...
		spcars = true
		sampSendChat('/apanel')
sampSendChat('/aad [Спавн Транспорта] Спавн транспорта произошел успешно | Приятной игры!')		
	end)
	else
...
cfg.main.lovlyareporta = false
	...
	if cfg.main.autologin then
		if dialogText:find('Неверный пароль') then
			...
			sampSendDialogResponse(1, 1, 65535, cfg.main.akkpass)
            sampAddChatMessage('Автовход в аккаунт {32CD32} выполнен успешно!')
			return false
		end
	end
		ReconWindow.v = false
	...
	if id == 2934 and cfg.main.autoadm then
		sampSendDialogResponse(2934, 1, 65535, cfg.main.admpass)
        sampAddChatMessage('Автовход в админку {32CD32} выполнен успешно!')
		return false
	end
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnjailsLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnmutesLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnbanLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnbanipLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//BanoffLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//JailoffLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//MuteoffLogs.txt", 'a')
...
local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//BanipoffLogs.txt", 'a')
...
if not isGamePaused() then	
	...
	if text:find("(.*) %[(.*)%] купил бизнес ID: (.*) по гос. цене за (.*) ms! Капча: %((%d+) | (%d+)%)") and cfg.autoopra.settingbiz then
		...
		sampAddChatMessage(tag..'Используйте команду {E78284}/'..cmdBusiness.v..' {84A6E7}для запроса опровержения на бизнес {E78284}№'..bidbiz, -1)
		end
		local f = io.open(getGameDirectory().."\\moonloader\\InsaneTools\\Логи\\AutoOpraLogs.txt", 'a')
...
function gw()
	GiveawayWindow.v = not GiveawayWindow.v
	imgui.Process = GiveawayWindow.v
end
...
elseif logBut["selected"] == 5 then
	imgui.Text(u8'Активация авто-опровержения на дома и бизнесы: /autoopra, /aadpra')
	imgui.Separator()
...
function regcommands()
	...
	sampRegisterChatCommand("ath", ath)
	...
	sampRegisterChatCommand("tpmenu", tpm)
	sampRegisterChatCommand('spcars', spcars)
	...
	sampRegisterChatCommand('captcha', kapcha)
    sampRegisterChatCommand('giveaway', gw)
...
if not doesDirectoryExist(getGameDirectory()..'//moonloader//InsaneTools') then 
	createDirectory(getGameDirectory()..'//moonloader//InsaneTools') 
	sampAddChatMessage(tag..'Директория для нужных файлов скрипта успешно создана', -1)
end
if not doesDirectoryExist(getGameDirectory()..'//moonloader//InsaneTools//Логи//') then 
	createDirectory(getGameDirectory()..'//moonloader//InsaneTools//Логи//') 
	sampAddChatMessage(tag..'Директория для логов успешно создана', -1)
end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//binder1.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//binder1.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №1" успешно создан', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//binder2.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//binder2.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №2" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//binder3.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//binder3.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №3" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//binder4.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//binder4.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Собственный №4" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//AcceptForms.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//AcceptForms.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи принятых форм успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//GiveWarnsTK.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//GiveWarnsTK.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи выданных варнов за ТК успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//zawitabinder.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//zawitabinder.txt", 'w')
		f:write(u8'/aad [INFO] Уважаемые игроки, берегите свои аккаунты. Устанавливайте Ключ Безопастности\n/aad [INFO] Не давайте пароль от своего аккаунта никому, не скачивайте модификации с неизвестных сайтов.\n/aad [INFO] Напомню, администрация не несет ответственности за ваши аккаунты. Все в ваших руках!')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Защита аккаунтов" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//donatebinder.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//donatebinder.txt", 'w')
		f:write(u8'/aad [INFO] Уважаемые игроки, на данный момент на сервере действует акция X4 донат!\n/aad [INFO] Вся пополненная сумма увеличивается в 4 раза, не упустите возможность приобрести привилегии по низкой цене!\n/aad [INFO] Пополнение счета на нашем сайте - youtube-rp.space')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Множитель доната" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//rulesbinder.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//rulesbinder.txt", 'w')
		f:write(u8'/aad [INFO] Уважаемые игроки, советую ознакомиться с правилами сервера на нашем форуме - forum.youtube-rp.space\n/aad За нарушение правил сервера Вы будете строго наказаны, приятной игры!')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Правила сервера" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//discordbinder.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//discordbinder.txt", 'w')
		f:write(u8'/aad [INFO] Не хватает общения?  После сложного учебного дня хочется отдохнуть?\n/aad [INFO] Тогда присоединяйся к Дискорду нашего сервера! Адекватные игроки и модерация.\n/aad [INFO] Ссылка на наш Дискорд-канал: discord.com/7aRbVRYDfj')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Дискорд" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//groupbinder.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//groupbinder.txt", 'w')
		f:write(u8'/aad [INFO] Уважаемые игроки, хотите узнавать актуальную информацию самым первым?\n/aad [INFO] Для такого случая у нас есть паблик ВК - vk.com/youtuberp_news, узнавайте новости самым первым!')
	    f:close()
		sampAddChatMessage(tag..'Файл для биндера "Паблик VK" успешно создан', -1)
	end
	
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//AdminVipChatLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//AdminVipChatLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи сообщений от администраторов в VIP чате создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//UnjailsLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnjailsLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи выпусков из деморгана создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//UnmutesLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnmutesLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи снятий мутов создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//UnbanLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnbanLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи разблокировок создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//UnibanLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//UnibanLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи разблокировок IP создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//BanoffLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//BanoffLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи блокировок в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//JailoffLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//JailoffLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи деморганов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//MuteoffLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//MuteoffLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи мутов в оффлайне создан.', -1)
	end
	if not doesFileExist(getGameDirectory().."//moonloader//InsaneTools//Логи//BanipoffLogs.txt") then
	    local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//BanipoffLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи выдачи блокировок IP в оффлайне создан.', -1)
	end
	
	if not doesFileExist(getWorkingDirectory().."\\InsaneTools\\Логи\\AutoOpraLogs.txt") then
		local f = io.open(getGameDirectory().."//moonloader//InsaneTools//Логи//AutoOpraLogs.txt", 'w')
	    f:close()
		sampAddChatMessage(tag..'Файл для записи покупок имущества создан.', -1)
	end
	
	mybind1 = io.open(getGameDirectory().."//moonloader//InsaneTools//binder1.txt", "r")
	mybinder1.v = mybind1:read('*a')
	mybind1:close()
	
	mybind2 = io.open(getGameDirectory().."//moonloader//InsaneTools//binder2.txt", "r")
	mybinder2.v = mybind2:read('*a')
	mybind2:close()
	
	mybind3 = io.open(getGameDirectory().."//moonloader//InsaneTools//binder3.txt", "r")
	mybinder3.v = mybind3:read('*a')
	mybind3:close()
	
	mybind4 = io.open(getGameDirectory().."//moonloader//InsaneTools//binder4.txt", "r")
	mybinder4.v = mybind4:read('*a')
	mybind4:close()
	
	zawitabind = io.open(getGameDirectory().."//moonloader//InsaneTools//zawitabinder.txt", "r")
	zawitabinder.v = zawitabind:read('*a')
	zawitabind:close()
	
	donatebind = io.open(getGameDirectory().."//moonloader//InsaneTools//donatebinder.txt", "r")
	donatebinder.v = donatebind:read('*a')
	donatebind:close()
	
	rulesbind = io.open(getGameDirectory().."//moonloader//InsaneTools//rulesbinder.txt", "r")
	rulesbinder.v = rulesbind:read('*a')
	rulesbind:close()
	
	discordbind = io.open(getGameDirectory().."//moonloader//InsaneTools//discordbinder.txt", "r")
	discordbinder.v = discordbind:read('*a')
	discordbind:close()
	
	groupbind = io.open(getGameDirectory().."//moonloader//InsaneTools//groupbinder.txt", "r")
...
IMAGES = {
    KILL_LIST = {
	[0] = "moonloader/InsaneTools/KillList/weap_0.png",
	[1] = "moonloader/InsaneTools/KillList/weap_1.png",
	[2] = "moonloader/InsaneTools/KillList/weap_2.png",
	[3] = "moonloader/InsaneTools/KillList/weap_3.png",
	[4] = "moonloader/InsaneTools/KillList/weap_4.png",
	[5] = "moonloader/InsaneTools/KillList/weap_5.png",
	[6] = "moonloader/InsaneTools/KillList/weap_6.png",
	[7] = "moonloader/InsaneTools/KillList/weap_7.png",
	[8] = "moonloader/InsaneTools/KillList/weap_8.png",
	[9] = "moonloader/InsaneTools/KillList/weap_9.png",
	[10] = "moonloader/InsaneTools/KillList/weap_10.png",
	[11] = "moonloader/InsaneTools/KillList/weap_11.png",
	[12] = "moonloader/InsaneTools/KillList/weap_12.png",
	[13] = "moonloader/InsaneTools/KillList/weap_13.png",
	[14] = "moonloader/InsaneTools/KillList/weap_14.png",
	[15] = "moonloader/InsaneTools/KillList/weap_15.png",
	[16] = "moonloader/InsaneTools/KillList/weap_16.png",
	[17] = "moonloader/InsaneTools/KillList/weap_17.png",
	[18] = "moonloader/InsaneTools/KillList/weap_18.png",
	[19] = "moonloader/InsaneTools/KillList/weap_19.png",
	[20] = "moonloader/InsaneTools/KillList/weap_20.png",
	[21] = "moonloader/InsaneTools/KillList/weap_21.png",
	[22] = "moonloader/InsaneTools/KillList/weap_22.png",
	[23] = "moonloader/InsaneTools/KillList/weap_23.png",
	[24] = "moonloader/InsaneTools/KillList/weap_24.png",
	[25] = "moonloader/InsaneTools/KillList/weap_25.png",
	[26] = "moonloader/InsaneTools/KillList/weap_26.png",
	[27] = "moonloader/InsaneTools/KillList/weap_27.png",
	[28] = "moonloader/InsaneTools/KillList/weap_28.png",
	[29] = "moonloader/InsaneTools/KillList/weap_29.png",
	[30] = "moonloader/InsaneTools/KillList/weap_30.png",
	[31] = "moonloader/InsaneTools/KillList/weap_31.png",
	[32] = "moonloader/InsaneTools/KillList/weap_32.png",
	[33] = "moonloader/InsaneTools/KillList/weap_33.png",
	[34] = "moonloader/InsaneTools/KillList/weap_34.png",
	[35] = "moonloader/InsaneTools/KillList/weap_35.png",
	[36] = "moonloader/InsaneTools/KillList/weap_36.png",
	[37] = "moonloader/InsaneTools/KillList/weap_37.png",
	[38] = "moonloader/InsaneTools/KillList/weap_38.png",
	[39] = "moonloader/InsaneTools/KillList/weap_39.png",
	[40] = "moonloader/InsaneTools/KillList/weap_40.png",
	[41] = "moonloader/InsaneTools/KillList/weap_41.png",
	[42] = "moonloader/InsaneTools/KillList/weap_42.png",
	[43] = "moonloader/InsaneTools/KillList/weap_43.png",
	[44] = "moonloader/InsaneTools/KillList/weap_44.png",
	[45] = "moonloader/InsaneTools/KillList/weap_45.png",
	[46] = "moonloader/InsaneTools/KillList/weap_46.png",
	[49] = "moonloader/InsaneTools/KillList/weap_49.png",
	[50] = "moonloader/InsaneTools/KillList/weap_50.png",
	[51] = "moonloader/InsaneTools/KillList/weap_51.png",
	[53] = "moonloader/InsaneTools/KillList/weap_53.png",
	[54] = "moonloader/InsaneTools/KillList/weap_54.png",
	[255] = "moonloader/InsaneTools/KillList/weap_255.png"
...
function checkKey()
		sampAddChatMessage("", -1) 
		sampAddChatMessage(tag ..  'Версия скрипта:{E78284} 24.07.2023', -1) 
		sampAddChatMessage(tag ..  'ВК:{E78284} bit.ly/InsaneTools', -1) 
		sampAddChatMessage("", -1) 
        sampAddChatMessage(tag .. '/amenu & ' .. klavisha .. ' {E78284}- AdminTools', -1)
        sampAddChatMessage(tag .. '/ath {E78284}- AdminTools Commands', -1)
        sampAddChatMessage(tag .. 'Developer: ImInsane *garant_xwareov', -1)
        sampAddChatMessage(tag .. 'This script only works on: {E78284}YouTube RP', -1)
end
